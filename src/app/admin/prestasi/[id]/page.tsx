...
export default function PrestasiEditPage() {
  ...
  useEffect(() => {
    getAchievementById(id).then(a => {
      if (a) {
        ...
        setImageId((a.imageUrl || "").match(/[?&]id=([^&]+)/)?.[1] || (a.imageUrl || "").match(/\/d\/([^/]+)/)?.[1] || a.imageUrl)
  ...
  if (loading) return <div className="space-y-6"><div className="h-8 w-40 bg-muted animate-pulse rounded" /><div className="h-96 bg-muted animate-pulse rounded-xl" /></div>
